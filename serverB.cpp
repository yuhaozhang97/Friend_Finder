...
#define UDP_PORT 31472
#define MAX_SIZE 1024


// Global variables
int serverB_socket_fd;
string request;
...
void initialize_socket() {
	serverB_socket_fd = socket(AF_INET, SOCK_DGRAM, 0);
	if(serverB_socket_fd == -1){
...
void initialize_sockaddr_structure() {
	...
	serverB_addr.sin_port = htons(UDP_PORT);
	serverB_addr.sin_addr.s_addr = inet_addr(LOCAL_HOST);
	// memset(&serverB_addr.sin_zero, 0, 8);
}

// Bind socket with serverB address
void bind_socket() {
	int nRet = bind(serverB_socket_fd, (struct sockaddr*) &serverB_addr, sizeof(sockaddr));
	if (nRet == -1) {
		perror("Server B failed to bind local port.");
		exit(1);
	}

	cout << "The server B is up and running using UDP on port " << UDP_PORT << endl << endl;
}
...
int main() {
	...
	string serverB_countries;
	for (const auto& country : country_user_map) {
		serverB_countries += country.first + " ";
	}
	...
	while (true) {
		...
		if (recvfrom(serverB_socket_fd, receive_buff, sizeof(receive_buff), 0, (struct sockaddr*) &client_addr, &client_addr_size) == -1) {
		...
		if (request == "B_country_list") {
			strncpy(send_buff, serverB_countries.c_str(), MAX_SIZE);
			if (sendto(serverB_socket_fd, send_buff, sizeof(send_buff), 0, (struct sockaddr*) &client_addr, sizeof(client_addr)) == -1) {
				perror("Failed to send country list to the main server.");
				exit(1);
			}
			cout << "The server B has sent a country list to Main Server" << endl << endl;
		} else {
			...
			cout << "The server B has received request for finding possible friends of User " << user << " in " << country << endl << endl;
			...
			if (user_map.find(user) == user_map.end()) {
				...
				cout << "The server B has sent \"User " << user << " not found\" to Main Server" << endl << endl;
			} else {
				// Found user
				...
				cout << "The server B has sent the result to Main Server" << endl << endl;
			}
		}
	}
	

	close(serverB_socket_fd);
	return 0;
}
